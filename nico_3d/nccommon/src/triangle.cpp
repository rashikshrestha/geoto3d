...
long incircle2count;                   /* Number of incircle2 tests performed. */
...
"        each triangle, and is useful for refining a finite element mesh\n");
  printf(
"        based on a posteriori error estimate2s.  A .poly file can optionally\n"
);
  printf(
...
"  meshes suitable for multigrid methods.  They also allow you to produce a\n"
  ...
  printf(
"  sequence of meshes using error estimate2-driven mesh refinement.\n");
  printf("\n");
  printf(
...
"  refine an existing mesh, and are typically based on a posteriori error\n");
  printf(
"  estimate2s resulting from a finite element simulation on that mesh.\n");
  printf("\n");
  printf(
...
"  (or -rpNEP if you've got a .poly file for the existing mesh) will\n");
  ...
  printf(
"  The -V switch produces extended statistics, including a rough estimate2\n");
  printf(
...
"  Triangle uses adaptive exact arithmetic to perform what computational\n");
  printf(
"  geometers call the `orientation' and `incircle2' tests.  If the floating-\n"
);
  printf(
...
"  routines do not ameliorate out-of-bounds exponents, which can arise\n");
  printf(
"  during the orientation and incircle2 tests.  As a rule of thumb, you\n");
  printf(
...
"    floating exception due to overflow when attempting to perform an\n");
  printf(
"    orientation or incircle2 test.  (Read the section on exact arithmetic\n");
  printf(
...
/*****************************************************************************/
/*                                                                           */
/*  exactinit2()   Initialize the variables used for exact arithmetic.        */
/*                                                                           */
...
void exactinit2()
{
  ...
  /* Error bounds for orientation and incircle2 tests. */
...
/*****************************************************************************/
/*                                                                           */
/*  fast_expansion_sum_zeroelim2()   Sum two expansions, eliminating zero     */
...
int fast_expansion_sum_zeroelim2(int elen, REAL *e, int flen, REAL *f, REAL *h)  /* h cannot be e or f. */
{
  REAL Q;
  INEXACT REAL Qnew;
  INEXACT REAL hh;
...
/*****************************************************************************/
/*                                                                           */
/*  scale_expansion_zeroelim2()   Multiply an expansion by a scalar,          */
...
int scale_expansion_zeroelim2(int elen, REAL *e, REAL b, REAL *h)   /* e and h cannot be the same. */
{
  INEXACT REAL Q, sum;
  REAL hh;
...
/*****************************************************************************/
/*                                                                           */
/*  estimate2()   Produce a one-word estimate2 of an expansion's value.        */
/*                                                                           */
/*  See my Robust Predicates paper for details.                              */
/*                                                                           */
/*****************************************************************************/

REAL estimate2(int elen, REAL *e)
{
  REAL Q;
  int eindex;
...
REAL counterclockwiseadapt(point pa, point pb, point pc, REAL detsum)
{
  ...
  u[3] = u3;
  C1length = fast_expansion_sum_zeroelim2(4, B, 4, u, C1);

  Two_Product(acx, bcytail, s1, s0);
  Two_Product(acy, bcxtail, t1, t0);
  Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
  u[3] = u3;
  C2length = fast_expansion_sum_zeroelim2(C1length, C1, 4, u, C2);

  Two_Product(acxtail, bcytail, s1, s0);
  Two_Product(acytail, bcxtail, t1, t0);
  Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
  u[3] = u3;
  Dlength = fast_expansion_sum_zeroelim2(C2length, C2, 4, u, D);
...
/*****************************************************************************/
/*                                                                           */
/*  incircle2()   Return a positive value if the point pd lies inside the     */
...
REAL incircle2adapt2(point pa, point pb, point pc, point pd, REAL permanent)
{
  ...
  bc[3] = bc3;
  axbclen = scale_expansion_zeroelim2(4, bc, adx, axbc);
  axxbclen = scale_expansion_zeroelim2(axbclen, axbc, adx, axxbc);
  aybclen = scale_expansion_zeroelim2(4, bc, ady, aybc);
  ayybclen = scale_expansion_zeroelim2(aybclen, aybc, ady, ayybc);
  alen = fast_expansion_sum_zeroelim2(axxbclen, axxbc, ayybclen, ayybc, adet);

  Two_Product(cdx, ady, cdxady1, cdxady0);
  Two_Product(adx, cdy, adxcdy1, adxcdy0);
  Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1], ca[0]);
  ca[3] = ca3;
  bxcalen = scale_expansion_zeroelim2(4, ca, bdx, bxca);
  bxxcalen = scale_expansion_zeroelim2(bxcalen, bxca, bdx, bxxca);
  bycalen = scale_expansion_zeroelim2(4, ca, bdy, byca);
  byycalen = scale_expansion_zeroelim2(bycalen, byca, bdy, byyca);
  blen = fast_expansion_sum_zeroelim2(bxxcalen, bxxca, byycalen, byyca, bdet);

  Two_Product(adx, bdy, adxbdy1, adxbdy0);
  Two_Product(bdx, ady, bdxady1, bdxady0);
  Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1], ab[0]);
  ab[3] = ab3;
  cxablen = scale_expansion_zeroelim2(4, ab, cdx, cxab);
  cxxablen = scale_expansion_zeroelim2(cxablen, cxab, cdx, cxxab);
  cyablen = scale_expansion_zeroelim2(4, ab, cdy, cyab);
  cyyablen = scale_expansion_zeroelim2(cyablen, cyab, cdy, cyyab);
  clen = fast_expansion_sum_zeroelim2(cxxablen, cxxab, cyyablen, cyyab, cdet);

  ablen = fast_expansion_sum_zeroelim2(alen, adet, blen, bdet, abdet);
  finlength = fast_expansion_sum_zeroelim2(ablen, abdet, clen, cdet, fin1);

  det = estimate2(finlength, fin1);
  ...
  if (adxtail != 0.0) {
	axtbclen = scale_expansion_zeroelim2(4, bc, adxtail, axtbc);
	temp16alen = scale_expansion_zeroelim2(axtbclen, axtbc, 2.0 * adx,
                                          temp16a);

	axtcclen = scale_expansion_zeroelim2(4, cc, adxtail, axtcc);
	temp16blen = scale_expansion_zeroelim2(axtcclen, axtcc, bdy, temp16b);

	axtbblen = scale_expansion_zeroelim2(4, bb, adxtail, axtbb);
	temp16clen = scale_expansion_zeroelim2(axtbblen, axtbb, -cdy, temp16c);

	temp32alen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                            temp16blen, temp16b, temp32a);
	temp48len = fast_expansion_sum_zeroelim2(temp16clen, temp16c,
                                            temp32alen, temp32a, temp48);
	finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
                                            temp48, finother);
    finswap = finnow; finnow = finother; finother = finswap;
  }
  if (adytail != 0.0) {
	aytbclen = scale_expansion_zeroelim2(4, bc, adytail, aytbc);
	temp16alen = scale_expansion_zeroelim2(aytbclen, aytbc, 2.0 * ady,
                                          temp16a);

	aytbblen = scale_expansion_zeroelim2(4, bb, adytail, aytbb);
	temp16blen = scale_expansion_zeroelim2(aytbblen, aytbb, cdx, temp16b);

	aytcclen = scale_expansion_zeroelim2(4, cc, adytail, aytcc);
	temp16clen = scale_expansion_zeroelim2(aytcclen, aytcc, -bdx, temp16c);
  ...
  if (bdxtail != 0.0) {
	bxtcalen = scale_expansion_zeroelim2(4, ca, bdxtail, bxtca);
	temp16alen = scale_expansion_zeroelim2(bxtcalen, bxtca, 2.0 * bdx,
                                          temp16a);

	bxtaalen = scale_expansion_zeroelim2(4, aa, bdxtail, bxtaa);
	temp16blen = scale_expansion_zeroelim2(bxtaalen, bxtaa, cdy, temp16b);

	bxtcclen = scale_expansion_zeroelim2(4, cc, bdxtail, bxtcc);
	temp16clen = scale_expansion_zeroelim2(bxtcclen, bxtcc, -ady, temp16c);
  ...
  if (bdytail != 0.0) {
	bytcalen = scale_expansion_zeroelim2(4, ca, bdytail, bytca);
	temp16alen = scale_expansion_zeroelim2(bytcalen, bytca, 2.0 * bdy,
                                          temp16a);

	bytcclen = scale_expansion_zeroelim2(4, cc, bdytail, bytcc);
	temp16blen = scale_expansion_zeroelim2(bytcclen, bytcc, adx, temp16b);

	bytaalen = scale_expansion_zeroelim2(4, aa, bdytail, bytaa);
	temp16clen = scale_expansion_zeroelim2(bytaalen, bytaa, -cdx, temp16c);
  ...
  if (cdxtail != 0.0) {
	cxtablen = scale_expansion_zeroelim2(4, ab, cdxtail, cxtab);
	temp16alen = scale_expansion_zeroelim2(cxtablen, cxtab, 2.0 * cdx,
                                          temp16a);

	cxtbblen = scale_expansion_zeroelim2(4, bb, cdxtail, cxtbb);
	temp16blen = scale_expansion_zeroelim2(cxtbblen, cxtbb, ady, temp16b);

	cxtaalen = scale_expansion_zeroelim2(4, aa, cdxtail, cxtaa);
	temp16clen = scale_expansion_zeroelim2(cxtaalen, cxtaa, -bdy, temp16c);
  ...
  if (cdytail != 0.0) {
	cytablen = scale_expansion_zeroelim2(4, ab, cdytail, cytab);
	temp16alen = scale_expansion_zeroelim2(cytablen, cytab, 2.0 * cdy,
                                          temp16a);

	cytaalen = scale_expansion_zeroelim2(4, aa, cdytail, cytaa);
	temp16blen = scale_expansion_zeroelim2(cytaalen, cytaa, bdx, temp16b);

	cytbblen = scale_expansion_zeroelim2(4, bb, cdytail, cytbb);
	temp16clen = scale_expansion_zeroelim2(cytbblen, cytbb, -adx, temp16c);
  ...
  if ((adxtail != 0.0) || (adytail != 0.0)) {
    if ((bdxtail != 0.0) || (bdytail != 0.0)
      ...
      v[3] = v3;
	  bctlen = fast_expansion_sum_zeroelim2(4, u, 4, v, bct);
    ...
    if (adxtail != 0.0) {
	  temp16alen = scale_expansion_zeroelim2(axtbclen, axtbc, adxtail, temp16a);
	  axtbctlen = scale_expansion_zeroelim2(bctlen, bct, adxtail, axtbct);
	  temp32alen = scale_expansion_zeroelim2(axtbctlen, axtbct, 2.0 * adx,
                                            temp32a);
	  temp48len = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
                                              temp48, finother);
      finswap = finnow; finnow = finother; finother = finswap;
      if (bdytail != 0.0) {
		temp8len = scale_expansion_zeroelim2(4, cc, adxtail, temp8);
		temp16alen = scale_expansion_zeroelim2(temp8len, temp8, bdytail,
                                              temp16a);
		finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp16alen,
                                                temp16a, finother);
        finswap = finnow; finnow = finother; finother = finswap;
      }
      if (cdytail != 0.0) {
		temp8len = scale_expansion_zeroelim2(4, bb, -adxtail, temp8);
		temp16alen = scale_expansion_zeroelim2(temp8len, temp8, cdytail,
                                              temp16a);
		finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp16alen,
                                                temp16a, finother);
        finswap = finnow; finnow = finother; finother = finswap;
	  ...
	  temp32alen = scale_expansion_zeroelim2(axtbctlen, axtbct, adxtail,
                                            temp32a);
	  axtbcttlen = scale_expansion_zeroelim2(bcttlen, bctt, adxtail, axtbctt);
	  temp16alen = scale_expansion_zeroelim2(axtbcttlen, axtbctt, 2.0 * adx,
                                            temp16a);
	  temp16blen = scale_expansion_zeroelim2(axtbcttlen, axtbctt, adxtail,
                                            temp16b);
	  temp32blen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
	  temp64len = fast_expansion_sum_zeroelim2(temp32alen, temp32a,
                                              temp32blen, temp32b, temp64);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp64len,
                                              temp64, finother);
      finswap = finnow; finnow = finother; finother = finswap;
    }
    if (adytail != 0.0) {
	  temp16alen = scale_expansion_zeroelim2(aytbclen, aytbc, adytail, temp16a);
	  aytbctlen = scale_expansion_zeroelim2(bctlen, bct, adytail, aytbct);
	  temp32alen = scale_expansion_zeroelim2(aytbctlen, aytbct, 2.0 * ady,
                                            temp32a);
	  temp48len = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
	  ...
	  temp32alen = scale_expansion_zeroelim2(aytbctlen, aytbct, adytail,
                                            temp32a);
	  aytbcttlen = scale_expansion_zeroelim2(bcttlen, bctt, adytail, aytbctt);
	  temp16alen = scale_expansion_zeroelim2(aytbcttlen, aytbctt, 2.0 * ady,
                                            temp16a);
	  temp16blen = scale_expansion_zeroelim2(aytbcttlen, aytbctt, adytail,
                                            temp16b);
	  temp32blen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
	  temp64len = fast_expansion_sum_zeroelim2(temp32alen, temp32a,
  ...
  if ((bdxtail != 0.0) || (bdytail != 0.0)) {
    if ((cdxtail != 0.0) || (cdytail != 0.0)
      ...
      Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
      v[3] = v3;
	  catlen = fast_expansion_sum_zeroelim2(4, u, 4, v, cat);
    ...
    if (bdxtail != 0.0) {
	  temp16alen = scale_expansion_zeroelim2(bxtcalen, bxtca, bdxtail, temp16a);
	  bxtcatlen = scale_expansion_zeroelim2(catlen, cat, bdxtail, bxtcat);
	  temp32alen = scale_expansion_zeroelim2(bxtcatlen, bxtcat, 2.0 * bdx,
                                            temp32a);
	  temp48len = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
      ...
      finswap = finnow; finnow = finother; finother = finswap;
      if (cdytail != 0.0) {
		temp8len = scale_expansion_zeroelim2(4, aa, bdxtail, temp8);
		temp16alen = scale_expansion_zeroelim2(temp8len, temp8, cdytail,
                                              temp16a);
		finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp16alen,
                                                temp16a, finother);
      ...
      if (adytail != 0.0) {
		temp8len = scale_expansion_zeroelim2(4, cc, -bdxtail, temp8);
		temp16alen = scale_expansion_zeroelim2(temp8len, temp8, adytail,
                                              temp16a);
		finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp16alen,
                                                temp16a, finother);
        finswap = finnow; finnow = finother; finother = finswap;
	  ...
	  temp32alen = scale_expansion_zeroelim2(bxtcatlen, bxtcat, bdxtail,
                                            temp32a);
	  bxtcattlen = scale_expansion_zeroelim2(cattlen, catt, bdxtail, bxtcatt);
	  temp16alen = scale_expansion_zeroelim2(bxtcattlen, bxtcatt, 2.0 * bdx,
                                            temp16a);
	  temp16blen = scale_expansion_zeroelim2(bxtcattlen, bxtcatt, bdxtail,
                                            temp16b);
	  temp32blen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
	  temp64len = fast_expansion_sum_zeroelim2(temp32alen, temp32a,
    ...
    if (bdytail != 0.0) {
	  temp16alen = scale_expansion_zeroelim2(bytcalen, bytca, bdytail, temp16a);
	  bytcatlen = scale_expansion_zeroelim2(catlen, cat, bdytail, bytcat);
	  temp32alen = scale_expansion_zeroelim2(bytcatlen, bytcat, 2.0 * bdy,
                                            temp32a);
	  temp48len = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
	  ...
	  temp32alen = scale_expansion_zeroelim2(bytcatlen, bytcat, bdytail,
                                            temp32a);
	  bytcattlen = scale_expansion_zeroelim2(cattlen, catt, bdytail, bytcatt);
	  temp16alen = scale_expansion_zeroelim2(bytcattlen, bytcatt, 2.0 * bdy,
                                            temp16a);
	  temp16blen = scale_expansion_zeroelim2(bytcattlen, bytcatt, bdytail,
                                            temp16b);
	  temp32blen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
	  temp64len = fast_expansion_sum_zeroelim2(temp32alen, temp32a,
  ...
  if ((cdxtail != 0.0) || (cdytail != 0.0)) {
    if ((adxtail != 0.0) || (adytail != 0.0)
      ...
      Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
      v[3] = v3;
	  abtlen = fast_expansion_sum_zeroelim2(4, u, 4, v, abt);
    ...
    if (cdxtail != 0.0) {
	  temp16alen = scale_expansion_zeroelim2(cxtablen, cxtab, cdxtail, temp16a);
	  cxtabtlen = scale_expansion_zeroelim2(abtlen, abt, cdxtail, cxtabt);
	  temp32alen = scale_expansion_zeroelim2(cxtabtlen, cxtabt, 2.0 * cdx,
                                            temp32a);
	  temp48len = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
      ...
      finswap = finnow; finnow = finother; finother = finswap;
      if (adytail != 0.0) {
		temp8len = scale_expansion_zeroelim2(4, bb, cdxtail, temp8);
		temp16alen = scale_expansion_zeroelim2(temp8len, temp8, adytail,
                                              temp16a);
		finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp16alen,
                                                temp16a, finother);
      ...
      if (bdytail != 0.0) {
		temp8len = scale_expansion_zeroelim2(4, aa, -cdxtail, temp8);
		temp16alen = scale_expansion_zeroelim2(temp8len, temp8, bdytail,
                                              temp16a);
		finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp16alen,
                                                temp16a, finother);
	  ...
	  temp32alen = scale_expansion_zeroelim2(cxtabtlen, cxtabt, cdxtail,
                                            temp32a);
	  cxtabttlen = scale_expansion_zeroelim2(abttlen, abtt, cdxtail, cxtabtt);
	  temp16alen = scale_expansion_zeroelim2(cxtabttlen, cxtabtt, 2.0 * cdx,
                                            temp16a);
	  temp16blen = scale_expansion_zeroelim2(cxtabttlen, cxtabtt, cdxtail,
                                            temp16b);
	  temp32blen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
	  temp64len = fast_expansion_sum_zeroelim2(temp32alen, temp32a,
    ...
    if (cdytail != 0.0) {
	  temp16alen = scale_expansion_zeroelim2(cytablen, cytab, cdytail, temp16a);
	  cytabtlen = scale_expansion_zeroelim2(abtlen, abt, cdytail, cytabt);
	  temp32alen = scale_expansion_zeroelim2(cytabtlen, cytabt, 2.0 * cdy,
                                            temp32a);
	  temp48len = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp32alen, temp32a, temp48);
	  finlength = fast_expansion_sum_zeroelim2(finlength, finnow, temp48len,
	  ...
	  temp32alen = scale_expansion_zeroelim2(cytabtlen, cytabt, cdytail,
                                            temp32a);
	  cytabttlen = scale_expansion_zeroelim2(abttlen, abtt, cdytail, cytabtt);
	  temp16alen = scale_expansion_zeroelim2(cytabttlen, cytabtt, 2.0 * cdy,
                                            temp16a);
	  temp16blen = scale_expansion_zeroelim2(cytabttlen, cytabtt, cdytail,
                                            temp16b);
	  temp32blen = fast_expansion_sum_zeroelim2(temp16alen, temp16a,
                                              temp16blen, temp16b, temp32b);
	  temp64len = fast_expansion_sum_zeroelim2(temp32alen, temp32a,
...
REAL incircle2(point pa, point pb, point pc, point pd)
{
  ...
  return incircle2adapt2(pa, pb, pc, pd, permanent);
}
...
void triangleinit()
{
  ...
  incircle2count = counterclockcount = hyperbolacount = 0;
  circumcentercount = circletopcount = 0;
  randomseed = 1;

  exactinit2();                     /* Initialize exact arithmetic constants. */
}
...
void checkdelaunay()
{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (shouldbedelaunay) {
		if (incircle2(triorg, tridest, triapex, oppoapex) > 0.0) {
...
void insertshelle(struct triedge *tri /* Edge at which to insert the new shell edge. */ , int shellemark /* Marker for the new shell edge. */)
{
  struct triedge oppotri;
    ...
    if (doflip) {
      ...
      if (top.tri == dummytri) {
        ...
        if ((leftpoint == infpoint1) || (leftpoint == infpoint2)
		  ...
		  doflip = incircle2(leftpoint, insertpoint, rightpoint, farpoint)
                   > 0.0;
        }
        if (doflip) {
...
void triangulatepolygon(struct triedge *firstedge, struct triedge *lastedge, int edgecount, int doflip, int triflaws)
{
  ...
  for (i = 2; i <= edgecount - 2; i++) {
	...
	if (incircle2(leftbasepoint, rightbasepoint, bestpoint, testpoint) > 0.0) {
  ...
  while (1) {
    ...
    if (!leftfinished) {
      ...
      if (nextapex != (point) NULL) {
		...
		badedge = incircle2(lowerleft, lowerright, upperleft, nextapex) > 0.0;
        while (badedge) {
          ...
          if (nextapex != (point) NULL) {
            /* Check whether the edge is Delaunay. */
			badedge = incircle2(lowerleft, lowerright, upperleft, nextapex)
                      > 0.0;
          } else {
    ...
    if (!rightfinished) {
      ...
      if (nextapex != (point) NULL) {
        /* Check whether the edge is Delaunay. */
		badedge = incircle2(lowerleft, lowerright, upperright, nextapex) > 0.0;
        while (badedge) {
          /* Eliminate the edge with an edge flip.  As a result, the     */
          ...
          if (nextapex != (point) NULL) {
            /* Check whether the edge is Delaunay. */
			badedge = incircle2(lowerleft, lowerright, upperright, nextapex)
                      > 0.0;
          } else {
            /* Avoid eating right through the triangulation. */
            badedge = 0;
    ...
    if (leftfinished || (!rightfinished &&
		   (incircle2(upperleft, lowerleft, lowerright, upperright) > 0.0))) {
...
void delaunayfixup(struct triedge *fixuptri, int leftside)
{
  ...
  if (counterclockwise(rightpoint, leftpoint, farpoint) > 0.0) {
	...
	if (incircle2(leftpoint, farpoint, rightpoint, nearpoint) <= 0.0) {
      return;
    }
...
void writenodes(char *nodefilename, int argc, char **argv)
...
void writeedges(char *edgefilename, int argc, char **argv)
...
void statistics()
{
  ...
  if (verbose) {
	...
	printf("  Number of incircle2 tests: %ld\n", incircle2count);
